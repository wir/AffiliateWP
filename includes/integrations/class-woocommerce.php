	...
	public function add_pending_referral( $order_id = 0, $posted ) {
		...
		if( $this->was_referred() || $affiliate_id ) {
			...
			if( $referral_id ) {
				...
				$message = apply_filters( 'affwp_' . $this->context . '_insert_payment_note', sprintf( __( 'Referral #%d for %s recorded for %s', 'affiliate-wp' ), $referral_id, $amount, $name ), $referral_id, $amount, $name, $order_id );
